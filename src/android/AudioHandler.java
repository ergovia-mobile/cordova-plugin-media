...
public class AudioHandler extends CordovaPlugin {
    ...
    protected void getWritePermission(final int requestCode) {
        PermissionHelper.requestPermission(this, requestCode, permissions[WRITE_EXTERNAL_STORAGE]);
    }


    protected void getMicPermission(final int requestCode) {
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("startRecordingAudio")) {
            ...
            final float f = this.getCurrentPositionAudio(args.getString(0));
            callbackContext.sendPluginResult(new PluginResult(status, f));
            return true;
        } else if (action.equals("getDurationAudio")) {
            final float f = this.getDurationAudio(args.getString(0), args.getString(1));
            callbackContext.sendPluginResult(new PluginResult(status, f));
            return true;
        } else if (action.equals("create")) {
            final String id = args.getString(0);
            final String src = FileHelper.stripFileProtocol(args.getString(1));
            getOrCreatePlayer(id, src);
        } else if (action.equals("release")) {
            final boolean b = this.release(args.getString(0));
    ...
    public void onDestroy() {
        ...
        for (final AudioPlayer audio : this.players.values()) {
            audio.destroy();
        }
        this.players.clear();
    }
    ...
    public Object onMessage(final String id, final Object data) {
        ...
        if (id.equals("telephone")) {
            ...
            // If phone idle, then resume playing those players we paused
                ...
                for (final AudioPlayer audio : this.pausedForPhone) {
    ...
    private AudioPlayer getOrCreatePlayer(final String id, final String file) {
     ...
     */
    private boolean release(final String id) {
        final AudioPlayer audio = players.remove(id);
        if (audio == null) {
            return false;
        }
        if (players.isEmpty()) {
     ...
     */
    public void startRecordingAudio(final String id, final String file) {
        final AudioPlayer audio = getOrCreatePlayer(id, file);
     ...
     */
    public void stopRecordingAudio(final String id, final boolean stop) {
        final AudioPlayer audio = this.players.get(id);
        if (audio != null) {
     ...
     * @param id The id of the audio player
     */
    public void resumeRecordingAudio(final String id) {
        final AudioPlayer audio = players.get(id);
        if (audio != null) {
     ...
     */
    public void seekToAudio(final String id, final int milliseconds) {
        final AudioPlayer audio = this.players.get(id);
        if (audio != null) {
     ...
     * @param id The id of the audio player
     */
    public void pausePlayingAudio(final String id) {
        final AudioPlayer audio = this.players.get(id);
        if (audio != null) {
            audio.pausePlaying();
        }
     ...
     * @param id The id of the audio player
     */
    public void stopPlayingAudio(final String id) {
        final AudioPlayer audio = this.players.get(id);
        if (audio != null) {
            audio.stopPlaying();
        }
     ...
     */
    public float getCurrentPositionAudio(final String id) {
        final AudioPlayer audio = this.players.get(id);
        if (audio != null) {
     ...
     */
    public float getDurationAudio(final String id, final String file) {
        final AudioPlayer audio = getOrCreatePlayer(id, file);
    ...
    public void setAudioOutputDevice(final int output) {
        final String TAG1 = "AudioHandler.setAudioOutputDevice(): Error : ";

        final AudioManager audiMgr = (AudioManager) this.cordova.getActivity().getSystemService(Context.AUDIO_SERVICE);
        if (output == 2) {
    ...
    public void resumeAllGainedFocus() {
        for (final AudioPlayer audio : this.pausedForFocus) {
            audio.startPlaying(null);
        }
    ...
    public void getAudioFocus() {
        final String TAG2 = "AudioHandler.getAudioFocus(): Error : ";

        final AudioManager am = (AudioManager) this.cordova.getActivity().getSystemService(Context.AUDIO_SERVICE);
        final int result = am.requestAudioFocus(focusChangeListener,
     ...
     */
    public void setVolume(final String id, final float volume) {
        final String TAG3 = "AudioHandler.setVolume(): Error : ";
    ...
    void sendEventMessage(final String action, final JSONObject actionData) {
        ...
        final PluginResult pluginResult = new PluginResult(PluginResult.Status.OK, message);
    ...
    public void onRequestPermissionResult(final int requestCode, final String[] permissions,
                                          final int[] grantResults) throws JSONException {
